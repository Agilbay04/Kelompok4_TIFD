        ...
        if(isset($_SESSION['daftar_laptop']))
        {
            ...
            if(mysqli_num_rows($query_get_latest_do) > 0)
            {
                while($row = mysqli_fetch_assoc($query_get_latest_do))
                {
                    ...
                    $raw_id_transaksi = $row['ID_TRANSAKSI'];
                    $processed_id_transaksi = intval(substr($raw_id_transaksi,3)) + 1;

                    if(strlen($processed_id_transaksi) == 1)
                    {
                        $id_transaksi = "TRS000".$processed_id_transaksi;
                    }else if(strlen($processed_id_transaksi) == 2)
                    {
                        $id_transaksi = "TRS00".$processed_id_transaksi;
                    }else if(strlen($processed_id_transaksi) == 3)
                    {
                        $id_transaksi = "TRS0".$processed_id_transaksi;
                    }else if(strlen($processed_id_transaksi) == 4)
                    {
                        $id_transaksi = "TRS".$processed_id_transaksi;
                    }
